...
function Terminal() {
  ...
  const codeStyle: React.CSSProperties = { ...mono, fontSize: 13, lineHeight: 1.7, background: "#fff", border: "2px solid #000", padding: "28px 28px", overflowX: "auto" };
  ...
  return (
    <div style={codeStyle}>
      ...
      {s >= 4 && <div style={dim}>[cachellm] breakpoints: system(2100tk) + tools(800tk)</div>}
      {s >= 5 && <div style={{ fontWeight: 700, color: "#1a1a1a" }}>[cachellm] cache hit — 2,100 tokens at 90% discount</div>}
      {s >= 6 && (
        <pre style={{ marginTop: 20, borderTop: "1px solid #e2e8f0", paddingTop: 16, ...mono, fontSize: 13, lineHeight: 1.6 }}>{
`┌───────────────────────────────────┐
│                                   │
│  cachellm                         │
│  requests:      48                │
...
function Section({ children, border = "hairline" }: { children: React.ReactNode; border?: "hairline" | "heavy" | "none" }) {
  const bb = border === "heavy" ? "2px solid #000" : border === "hairline" ? "1px solid #e2e8f0" : "none";
  return <section style={{ paddingTop: 48, paddingBottom: 48, borderBottom: bb }}>{children}</section>;
}

// ── Code block ───────────────────────────────────────────────────────
function Code({ children, heavy = false }: { children: React.ReactNode; heavy?: boolean }) {
  return (
    <div style={{ ...mono, fontSize: 13, lineHeight: 1.7, background: "#fafafa", border: heavy ? "2px solid #000" : "1px solid #e2e8f0", padding: 24, overflowX: "auto" }}>
      {children}
    </div>
  );
}
...
export default function Home() {
  ...
  return (
    <>
      ...
      <nav style={{ background: "#000", color: "#fff", padding: "0 32px" }}>
      ...
      <main style={{ maxWidth: 1200, margin: "0 auto", padding: "0 32px" }}>
        ...
        {/* ── HERO ── */}
        <header style={{ paddingTop: 64, paddingBottom: 48, borderBottom: "2px solid #000" }}>
          <div style={{ ...kicker, marginBottom: 8 }}>OPEN SOURCE · NPM PACKAGE</div>
          <h1 style={{ ...display, fontSize: "clamp(48px, 8vw, 80px)", marginBottom: 20 }}>cachellm</h1>
          <p style={{ ...bodySerif, maxWidth: 560, marginBottom: 36 }}>
            Auto-optimize LLM prompt caching. One line of code.{" "}
            <strong>60–90% savings</strong>{" "}on your Claude &amp; GPT API bills.
          </p>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
            <button onClick={copy} style={{ display: "inline-flex", alignItems: "center", gap: 12, padding: "12px 24px", border: "2px solid #000", borderRadius: 0, background: "#fff", color: "#000", ...mono, fontSize: 13, fontWeight: 700, cursor: "pointer", transition: "background 150ms, color 150ms" }}
            ...
            <a href="https://github.com/sahilempire/cachellm" target="_blank" style={{ display: "inline-flex", alignItems: "center", padding: "12px 24px", border: "2px solid #000", borderRadius: 0, background: "#fff", color: "#000", ...sans, fontSize: 14, fontWeight: 700, letterSpacing: "0.3px", textDecoration: "none", transition: "background 150ms, color 150ms" }}
              onMouseEnter={e => { e.currentTarget.style.background = "#000"; e.currentTarget.style.color = "#fff"; }}
              onMouseLeave={e => { e.currentTarget.style.background = "#fff"; e.currentTarget.style.color = "#000"; }}
            >VIEW SOURCE →</a>
          </div>
        ...
        <Section>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)" }}>
            {[
              ...
              <div key={i} style={{ paddingRight: 32, paddingLeft: i > 0 ? 32 : 0, borderLeft: i > 0 ? "1px solid #e2e8f0" : "none" }}>
                <div style={{ ...kickerLight, marginBottom: 8 }}>{stat.label}</div>
                <div style={{ ...display, fontSize: 44 }}>
        ...
        <Section>
          ...
          {[
            ...
            <div key={i} style={{ display: "flex", gap: 24, padding: "20px 0", borderBottom: i < 3 ? "1px solid #e2e8f0" : "none" }}>
              <span style={{ ...display, fontSize: 36, minWidth: 52, color: "#1a1a1a" }}>{step.n}</span>
              <div>
                <div style={{ ...kicker, marginBottom: 4 }}>{step.t}</div>
                <p style={{ ...sans, color: "#757575", fontSize: 15, lineHeight: 1.6 }}>{step.d}</p>
              </div>
            </div>
          ))}
        </Section>
            ...
            <thead>
              <tr style={{ borderBottom: "2px solid #000" }}>
                {["SCALE", "BEFORE", "AFTER", "SAVED/DAY"].map((h, i) => (
                  <td key={h} style={{ ...kickerLight, padding: "12px 0", textAlign: i > 0 ? "right" : "left" }}>{h}</td>
                ))}
              </tr>
            </thead>
            <tbody>
              {[
                ...
                <tr key={i} style={{ borderBottom: "1px solid #e2e8f0" }}>
                  ...
                  <td style={{ padding: "16px 0", textAlign: "right", color: "#757575", textDecoration: "line-through", fontSize: 14 }}>{r.before}</td>
                  <td style={{ padding: "16px 0", textAlign: "right", fontWeight: 700, fontSize: 17 }}>{r.after}</td>
                  <td style={{ padding: "16px 0", textAlign: "right", ...mono, fontSize: 12, color: "#757575" }}>{r.saved}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <Section>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr" }}>
            {[
              ...
              <div key={i} style={{ paddingRight: 28, paddingLeft: i > 0 ? 28 : 0, borderLeft: i > 0 ? "1px solid #000" : "none" }}>
        ...
        <Section>
          ...
          <div style={{ ...mono, fontSize: 12, lineHeight: 1.8, background: "#fff", border: "2px solid #000", padding: 28, overflowX: "auto" }}>
            <pre style={{ ...mono, fontSize: 12, lineHeight: 1.8 }}>{
`YOUR APP
  │
        ...
        <Section>
          ...
          {[
            ...
            <div key={i} style={{ display: "flex", gap: 40, padding: "20px 0", borderBottom: i < 3 ? "1px solid #e2e8f0" : "none", alignItems: "baseline" }}>
              <span style={{ ...kicker, minWidth: 200, flexShrink: 0 }}>{p.k}</span>
              <span style={{ ...sans, color: "#757575", fontSize: 15, lineHeight: 1.6 }}>{p.v}</span>
            </div>
          ))}
        </Section>

        {/* ── ROADMAP ── */}
        <section style={{ paddingTop: 48, paddingBottom: 48, borderBottom: "2px solid #000" }}>
          <Ribbon>ROADMAP</Ribbon>
          <div style={{ display: "flex", flexDirection: "column", gap: 12, ...sans, fontSize: 15 }}>
            {[
